	...
	Preview.prototype = {
		...
		update : function( $item ) {
			...
			var $itemEl = this.$item.children( 'a' ),
			     linkdata = $itemEl.data(),
				eldata = {
					href : $itemEl.attr( 'href' ),
					largesrc : linkdata['largesrc'],
					hugesrc : linkdata['hugesrc'],
					title : linkdata['title'],
                     description : linkdata['description'],
				};
				
				delete(linkdata['largesrc']);
				delete(linkdata['hugesrc']);
				delete(linkdata['title']);
                 delete(linkdata['description']);
				this.$infolist.empty();
                 this.$infolist.append($("<li><label>fid</label>: " + linkdata['fid'] + "</li>"));
				var fields = linkdata['fieldOrder'].split(",");
				for (var n in fields) {
				    var key = fields[n];
				    if (key == "title" || key == "description") { continue; }
				    this.$infolist.append($("<li><label>" + key + "</label>: " + linkdata[key] + "</li>"));
				}
				...
				$( '<img/>' ).load( function() {
					...
					if( $img.attr( 'src' ) === self.$item.children('a').data( 'largesrc' ) ) {
						...
						$("a.lightbox-link, a.lightbox-img-link").unbind('click').click(function(event) {
							...
							var options = { 
								index: iind, 
								getNumItemsFn: function() { return Drupal.settings.shanti_views_image_grid.total_items; }};
							...
							var iteminfo = Drupal.settings.shanti_views_image_grid,
